...
class DataSet(Data.Dataset):
    def __init__(self, __C, RUN_MODE, text_segment=None, spacy_tool=None):
        ...
        if self.use_tsv_feat:
            ...
            self.datasets_with_splits = ['conceptual']
            self.split_size = 800000
            self.tsv_files = {}
    ...
    # def check_tsv_feat(self):
    #     if self.tsv_files == {} or self.tsv_files is None:
    #         # self.datasets_with_splits = ['sbu', 'conceptual']
    #         self.datasets_with_splits = ['conceptual']
    #         self.split_size = 800000
    #         self.tsv_files = {}
    ...
    def __getitem__(self, idx):
        ...
        mask_prob_text, mask_prob_img = None, None
        ...
        text_do_mask_loss = False
        if is_text_masking:
            # Masking text
            text_do_mask_loss = not (not is_matched and 'text' in self.__C.MATCH_CONSTRAIN)
            text_input, text_outside_weight, mask_prob_text, mask_prob_img = self.masking_text(
                text_input, formatted_data_text, grain_label, imgfeat['boxes'], lemmas, text_do_mask_loss)
            if 'mlm' in self.__C.TASKS['text']:
                text_mlm_loss_valid = torch.tensor(1, dtype=torch.float32)
        
        # Padding and convert text ids
        text_input_ids, text_mask, text_outside_weight, text_mlm_label_ids, text_mlm_weight = self.proc_text(
            text_input, text_outside_weight, text_mlm_label)
        ...
        img_do_mask_loss = None
        if is_imgfeat_masking:
            # Masking img features
            # print(is_matched)
            img_do_mask_loss = not (not is_matched and 'image' in self.__C.MATCH_CONSTRAIN)
            imgfeat_input, imgfeat_outside_weight, mask_prob_text, mask_prob_img = self.masking_imgfeat(
                imgfeat_input, boxes, grain_label, formatted_data_text, tokenized_text, lemmas, img_do_mask_loss)
        ...
        if mask_prob_text is not None:
            for ix in range(len(mask_prob_text)):
                if random.random() < mask_prob_text[ix].item():
                    prob = random.random()
                    if prob < 0.8:
                        text_input_ids[ix+1] = 103
                    elif prob < 0.9:
                        text_input_ids[ix+1] = random.randint(104, len(list(self.tokenizer.vocab.items()))-1)
                    
                    if text_do_mask_loss:
                        text_outside_weight[ix+1] = 1.
        
        if mask_prob_img is not None:
            for ix in range(len(mask_prob_img)):
                if random.random() < mask_prob_img[ix].item():
                    prob = random.random()
                    if prob < 0.8:
                        imgfeat_input[ix, :] = 0.
                    elif prob < 0.9:
                        imgfeat_input[ix, :] = torch.from_numpy(self.rand_imgfeat_from_aggr(only_obj=True))
                    
                    if img_do_mask_loss:
                        imgfeat_outside_weight[ix] = 1.
    ...
    def masking_struct_tsg_1d(self, mlm_outside_weight, mlm_pos, tokenized_text, formatted_data, lemmas, do_mask_loss=True):
        ...
        mask_ids = []
        mat_choice = set()
        prob_mat_00 = torch.zeros([len(tokenized_text), len(tokenized_text)], dtype=torch.float32)
        for objs_id in objs_ids:
            if random.random() < (self.__C.MASK_STRUCT_PROB['tsg'] + 0.2):
                if not (objs_id[0][0] < len(bpe_ids)):
                    continue
                for sent_i in bpe_ids[objs_id[0][0]]:
                    mask_ids.append(sent_i)
                ...
                if cent in attr_map:
                    for cent_i, cent_v in enumerate(attr_map[cent]):
                        ...
                        for ind in inds:
                            if objs_id[0][0] < len(bpe_ids) and ind < len(bpe_ids):
                                for sent_i in bpe_ids[objs_id[0][0]]:
                                    for sent_j in bpe_ids[ind]:
                                        mat_choice.add(sent_i)
                                        mat_choice.add(sent_j)
                                        prob_mat_00[sent_i][sent_j] = prob
                                        prob_mat_00[sent_j][sent_i] = prob
        ...
        return mask_ids, mat_choice, prob_mat_00
    ...
    def masking_text(self, tokenized_text, formatted_data, grain_label, boxes, lemmas, do_mask_loss=True):
        ...
        if self.__C.MASK_STRUCT['tsg']:
            mask_ids, mat_choice_00, prob_mat_00 = self.masking_struct_tsg_1d(mlm_outside_weight, mlm_pos, masked_tokenized_text, formatted_data, lemmas, do_mask_loss)
        
        mat_choice_01 = set()
        mat_choice_10 = set()
        mat_choice_11 = set()
        prob_mat_01 = torch.zeros([len(masked_tokenized_text), grain_label.size(1)], dtype=torch.float32)
        prob_mat_10 = torch.zeros([grain_label.size(1), len(masked_tokenized_text)], dtype=torch.float32)
        prob_mat_11 = torch.zeros([grain_label.size(1), grain_label.size(1)], dtype=torch.float32)
        ...
        for i in mask_ids:
            ...
            for k in keep.tolist():
                prob_mat_01[i, k] = grain_label[i, k]
                prob_mat_10[k, i] = grain_label[i, k]
                mat_choice_01.add(i)
                mat_choice_10.add(k)
        
        mask_prob_text = torch.zeros(len(masked_tokenized_text))
        mask_prob_img = torch.zeros(len(boxes))
        if len(mat_choice_10) == 0:
            for i in mask_ids:
                mask_prob_text[i] = 1.0
        else:
            ...
            for ki in range(grain_label.size(1)):
                ...
                if ious[k][ki] > 0.2:
                    prob = ious[k][ki]
                    prob_mat_11[k, ki] = prob
                    prob_mat_11[ki, k] = prob
                    mat_choice_11.add(k)
                    mat_choice_11.add(ki)
        
            mat_choice_00_list = list(mat_choice_00)
            mat_choice_01_list = list(mat_choice_01)
            mat_choice_10_list = list(mat_choice_10)
            mat_choice_11_list = list(mat_choice_11)

            if len(mat_choice_00) > 0:
                prob_mat_00[mat_choice_00_list] = prob_mat_00[mat_choice_00_list] / prob_mat_00[mat_choice_00_list].sum(dim=-1, keepdim=True)
            if len(mat_choice_01) > 0:
                prob_mat_01[mat_choice_01_list] = prob_mat_01[mat_choice_01_list] / prob_mat_01[mat_choice_01_list].sum(dim=-1, keepdim=True)
            if len(mat_choice_10) > 0:
                prob_mat_10[mat_choice_10_list] = prob_mat_10[mat_choice_10_list] / prob_mat_10[mat_choice_10_list].sum(dim=-1, keepdim=True)
            if len(mat_choice_11) > 0:
                prob_mat_11[mat_choice_11_list] = prob_mat_11[mat_choice_11_list] / prob_mat_11[mat_choice_11_list].sum(dim=-1, keepdim=True)
            
            mat_choice_00 = sorted(list(mat_choice_00))
            mat_choice_11 = mat_choice_10 | mat_choice_11
            mat_choice_01 = sorted(list(mat_choice_01))
            mat_choice_10 = sorted(list(mat_choice_10))
            mat_choice_11 = sorted(list(mat_choice_11))

            prob_mat_00 = prob_mat_00[mat_choice_00][:, mat_choice_00]
            prob_mat_01 = prob_mat_01[mat_choice_00][:, mat_choice_11]
            prob_mat_10 = prob_mat_10[mat_choice_11][:, mat_choice_00]
            prob_mat_11 = prob_mat_11[mat_choice_11][:, mat_choice_11]

            choice_to_ids = {}
            for ids in range(len(mat_choice_00)):
                choice_to_ids[mat_choice_00[ids]] = ids
            
            prob_mat = torch.cat([torch.cat([prob_mat_00, prob_mat_01], dim=1), torch.cat([prob_mat_10, prob_mat_11], dim=1)], dim=0)
            prob_mat = prob_mat / prob_mat.sum(dim=-1, keepdim=True)

            alpha = 0.8
            prob_mat = alpha * prob_mat + (1 - alpha) * torch.mm(prob_mat, prob_mat)

            for ids in mask_ids:
                if ids in choice_to_ids:
                    prob = prob_mat[choice_to_ids[ids], :]
                    prob_text = prob[:len(mat_choice_00)]
                    prob_img = prob[len(mat_choice_00):]

                    beta = 0.9
                    for i in range(prob_text.size(0)):
                        mask_prob_text[mat_choice_00[i]] = beta * prob_text[i].item()
                    mask_prob_text[ids] = 1.0

                    if len(prob_img) == 0 or prob_img.sum() == 0:
                        prob_img = None
                    else:
                        prob_img = 1 - prob_img
                        for i in range(prob_img.size(0)):
                            mask_prob_img[mat_choice_11[i]] = (1 - beta) * prob_img[i].item()
                
                else:
                    mask_prob_text[ids] = 1.0

        return masked_tokenized_text, mlm_outside_weight, mask_prob_text, mask_prob_img


    def proc_text(self, text_input, text_outside_weight, text_mlm_label):
        ...
        return text_input_ids, text_mask, text_outside_weight, text_mlm_label_ids, text_mlm_weight
    ...
    def masking_imgfeat(self, imgfeat_input, boxes, grain_label, formatted_data, tokenized_text, lemmas, do_mask_loss=True):
        ...
        mat_choice_11 = set()
        prob_mat_00 = torch.zeros([len(tokenized_text), len(tokenized_text)], dtype=torch.float32)
        prob_mat_01 = torch.zeros([len(tokenized_text), len(boxes)], dtype=torch.float32)
        prob_mat_10 = torch.zeros([len(boxes), len(tokenized_text)], dtype=torch.float32)
        prob_mat_11 = torch.zeros([len(boxes), len(boxes)], dtype=torch.float32)
        for i in range(imgfeat_input.shape[0]):
            if random.random() < mask_grain_weight_sum[i] * self.__C.OBJ_GRAIN_RATIO:
                mask_ids.append(i)
                for j in range(imgfeat_input.shape[0]):
                    if ious[i][j] > 0.2:
                        prob = ious[i][j]
                        prob_mat_11[i, j] = prob
                        prob_mat_11[j, i] = prob
                        mat_choice_11.add(i)
                        mat_choice_11.add(j)
        ...
        for i in mask_ids:
            keep = torch.where(grain_label[:, i] > 0.9)[0]
            for k in keep.tolist():
                prob_mat_01[k, i] = grain_label[k, i]
                prob_mat_10[i, k] = grain_label[k, i]
                mat_choice_01.add(k)
                mat_choice_10.add(i)
        
        mask_prob_text = torch.zeros(len(tokenized_text))
        mask_prob_img = torch.zeros(len(boxes))
        if len(mat_choice_01) == 0:
            for i in mask_ids:
                mask_prob_img[i] = 1.0
            ...
            for k in mat_choice_01:
                ...
                if cent in attr_map:
                    for cent_i, cent_v in enumerate(attr_map[cent]):
                        ...
                        inds = attr_map_ids[cent][cent_i]
                        for ind in inds:
                            if tid < len(bpe_ids) and ind < len(bpe_ids):
                                for sent_i in bpe_ids[tid]:
                                    for sent_j in bpe_ids[ind]:
                                        mat_choice_00.add(sent_i)
                                        mat_choice_00.add(sent_j)
                                        prob_mat_00[sent_i][sent_j] = prob
                                        prob_mat_00[sent_j][sent_i] = prob
            ...
            mat_choice_11 = sorted(list(mat_choice_11))
            mat_choice_00 = mat_choice_01 | mat_choice_00
            mat_choice_10 = sorted(list(mat_choice_10))
            mat_choice_01 = sorted(list(mat_choice_01))
            mat_choice_00 = sorted(list(mat_choice_00))
            ...
            choice_to_ids = {}
            for ids in range(len(mat_choice_11)):
                choice_to_ids[mat_choice_11[ids]] = ids + len(mat_choice_00)
            ...
            for ids in mask_ids:
                if ids in choice_to_ids:
                    ...
                    beta = 0.9
                    for i in range(prob_img.size(0)):
                        mask_prob_img[mat_choice_11[i]] = beta * prob_img[i].item()
                    mask_prob_img[ids] = 1.0

                    if len(prob_text) == 0 or prob_text.sum() == 0:
                        prob_text = None
                    else:
                        prob_text = 1 - prob_text
                        for i in range(prob_text.size(0)):
                            mask_prob_text[mat_choice_00[i]] = (1 - beta) * prob_text[i].item()
                
                else:
                    mask_prob_img[ids] = 1.0

        return masked_imgfeat_input, mlm_outside_weight, mask_prob_text, mask_prob_img
