...
@ray.remote(num_cpus=1)
def transfer_npz2row(npz_path, ray_split_num, ray_split_idx, npz_list, actor: ActorHandle):
    ...
    for i, npz_file in enumerate(npz_list):
        ...
        idx = i * ray_split_num + ray_split_idx
...
def transfer_npz2tsv(npz_path, tsv_path, ray_split_num):
    ...
    # img_splits_lists = [imglist[i::split_num] for i in range(split_num)]

    img_feat_offset_map = {}
    # for split_idx in range(split_num):
    # npz_split_list = npz_list[split_idx*split_size:(split_idx+1)*split_size]
    # npz_lists = [npz_split_list[i::num_gpus] for i in range(num_gpus)]
    # split_npzs = len(npz_split_list)
    # print('Starting transfer split {}.'.format(split_idx))
    # print('Number of npzs in this split:', split_npzs)
    # pb = ProgressBar(split_images)
    pb = ProgressBar(len(npz_list))
    npz_lists = [npz_list[i::ray_split_num] for i in range(ray_split_num)]
    actor = pb.actor
    ...
    for i in range(ray_split_num):
        ...
        transfer_row_list.append(transfer_npz2row.remote(npz_path, ray_split_num, i, npz_lists[i], actor))
    ...
    for i in range(len(npz_list)):
        ...
        rows[i] = rows_and_maps[part_idx][0][0]
...
if __name__ == '__main__':
    ...
    ray_split_num = 12
    transfer_npz2tsv(path_to_npz_files, path_to_tsv_files, ray_split_num)